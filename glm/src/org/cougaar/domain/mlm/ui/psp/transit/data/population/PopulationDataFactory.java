 ...
 * @since 1/24/01
 **/
public class PopulationDataFactory implements DeXMLableFactory{
  ...
  public DeXMLable beginSubObject(DeXMLable curObj, String name, 
    ...
    if(curObj==null && name.equals(PopulationData.NAME_TAG)){
      return new PopulationData();
    }else if((curObj instanceof PopulationData) && 
	     name.equals(ConveyancePrototype.NAME_TAG)){
      return new ConveyancePrototype();
    }else if((curObj instanceof PopulationData) && 
	     name.equals(ConveyanceInstance.NAME_TAG)){
      return new ConveyanceInstance();
    }
    return null;
  }
}
