/* $Header: /opt/rep/cougaar/glm/glm/src/org/cougaar/domain/mlm/ui/newtpfdd/util/Attic/IndexedTreeMap.java,v 1.1 2001-02-22 22:42:34 wseitz Exp $ */
...
/*
 * Ripped out of java.util (hence the imports) and extended to keep track of the
 * childCount of every node.  This allows finding the nth node of the ordered
 * set that would be generated by an in-order traversal, in log n time.  More
 * simply, we simulate, fairly cheaply, an array interface: "give me the
 * 15,234th element from this sorted collection." Right now, we only care about
 * the get() method but there's no reason this couldn't completely implement
 * Vector or what have you. Changes from TreeMap.java: match on // Fix up and
 * that does all the additional accounting; added childCount field to Entry;
 * added get(int index) method.
 *
 * Copyright 1999, by Ascent Technologies, Inc.
 * 64 Sidney St. Ste. 380, Cambridge, MA 02139-4136
 * All rights reserved.
 *
 * @author Daniel Bromberg
 */
	             ...
	             implements SortedMap, Cloneable, java.io.Serializable
{
     ...
     * value)</tt> call will throw a <tt>ClassCastException</tt>.
     */
    public IndexedTreeMap(Comparator c) {
	this.comparator = c;
    }
     ...
     * runs in linear time.
     */
    public IndexedTreeMap(SortedMap m) {
    ...
    /**
     * Returns the value that corresponds to the ith element of an inorder
     * traversal of the tree.  Returns <tt>null</tt> if the index is out
     * of range.  Added by Ascent.
     *
     * @param i index of desired element
     * @return the value corresponding to that index, or <tt>null</tt> if the
     *         index is out of range.
     */
    public Object get(int i)
    {
	return internalGet(root, i + 1);
    }

    private Object internalGet(Entry entry, int i)
    {
	int leftCount = entry.left == null ? 0 : entry.left.childCount;
	if ( leftCount + 1 == i )
	    return entry.value;
	if ( i <= leftCount ) {
	    if ( entry.left == null )
		return null;
	    return internalGet(entry.left, i);
	}
	// i > leftcount
	if ( entry.right == null )
	    return null;
	return internalGet(entry.right, i - leftCount - 1);
    }

    /**
     * Returns the index that corresponds to the location of the given element
     * in an inorder traversal of the tree.  Added by Ascent.
     *
     * @param entry the entry for which the index is desired
     * @return the index
     */
	...
	if ( entry == null )
	    return -1;
	return internalIndexOf(entry);
    }

    private int internalIndexOf(Entry entry)
    {
	int leftCount = entry.left == null ? 0 : entry.left.childCount;
	int rightCount = entry.right == null ? 0 : entry.right.childCount;
	if ( entry == root )
	    return leftCount;
	if ( entry == entry.parent.left )
	    return internalIndexOf(entry.parent) - rightCount - 1; // duh!
	// if not, entry is right child
	return internalIndexOf(entry.parent) + leftCount + 1;
    }
	...
	while (true) {
		...
		if (t.left != null) {
		    ...
		    // Fix up child counts;
		    for ( Entry ancestor = t; ancestor != null; ancestor = ancestor.parent )
			ancestor.childCount++;
     ...
     */
    public Object clone() {
        return new IndexedTreeMap(this);
    }
	...
	if (keySet == null) {
		...
		public boolean remove(Object o) {
		    return IndexedTreeMap.this.remove(o) != null;
		}
	...
	public boolean containsKey(Object key) {
	    return inRange(key) && IndexedTreeMap.this.containsKey(key);
	}
	...
	private class EntrySetView extends AbstractSet {
		...
		if (size == -1 || sizeModCount != IndexedTreeMap.this.modCount) {
		    size = 0;  sizeModCount = IndexedTreeMap.this.modCount;
	...
	p.parent = r;
	// Fix up child counts
	r.childCount = p.childCount;
	p.childCount = 1
	    + (p.right != null ? p.right.childCount : 0)
	    + (p.left  != null ? p.left .childCount : 0);
    }
	...
	// we now know at least one of left or right is null
	...
	if (replacement != null) {
	    ...
	    // Fix up child counts
	    for ( Entry ancestor = p.parent; ancestor != null; ancestor = ancestor.parent )
		ancestor.childCount--; // a single child disappeared from this branch
	...
	// Swap child counts
	int tempCount = x.childCount;
	x.childCount = y.childCount;
	y.childCount = tempCount;
	...
	// Fix up child counts
	middle.childCount = 1
	    + (middle.right != null ? middle.right.childCount : 0)
	    + (middle.left  != null ? middle.left .childCount : 0);
        return middle;
    }
    ...
    public void inOrder(Entry entry)
    {
	if ( entry == null )
	    return;
	inOrder(entry.left);
	System.out.println(entry.value
			   + " l: " + (entry.left != null ? entry.left.value : null)
			   + " r: " + (entry.right != null ? entry.right.value : null)
			   + " c: " + entry.childCount + " ");
	inOrder(entry.right);
    }
    ...
    {
	// a little test to take 'er around the block a few thousand times
	IndexedTreeMap testTree = new IndexedTreeMap();
	int[] elems = new int[100000];
	
	for ( int i = 0; i < 100000; i++ )
	    elems[i] = i;
	for ( int i = 0; i < 123456; i++ ) {
	    int j1 = (int)Math.round(Math.random() * 100000);
	    int j2 = (int)Math.round(Math.random() * 100000);
	    if ( j1 == 100000 )
		j1 = 0;
	    if ( j2 == 100000 )
		j2 = 0;
	...
	Integer foo;
	for ( int i = 0; i < 100000; i++ ) {
	    foo = new Integer(elems[i]);
	    System.out.print(foo.toString() + " ");
	    testTree.put(foo, "[" + foo.toString() + "]");
	}
	System.out.println("");
	...
	for ( int i = 0; i < 100000; i++ )
	    System.out.print((String)testTree.get(i) + " ");
	System.out.println("");

	for ( int i = 0; i < 50000; i++ )
	    testTree.remove(new Integer(elems[i]));
