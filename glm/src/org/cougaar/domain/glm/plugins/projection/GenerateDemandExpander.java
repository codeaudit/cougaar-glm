...
// 	    total_demand += rate.getPerDay()*duration;
	...
	if (ownSubscription_.elements().hasMoreElements()) {
	    printLog("updateGenerateDemandExpansion() has elements() added = "+
		     ownSubscription_.getAddedList().hasMoreElements()+" changed = "+
		     ownSubscription_.getChangedList().hasMoreElements() );
	}
	updateExpansionResult(ownSubscription_.elements());
    }
     ...
     * @returns number of days between orders for resources.  
     *          Defaults to one day.
     **/
